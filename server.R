    ...
    observe({
      if(input$submit1 > 0){
        isolate({
          ...
          ggmar <- function(){
            p <- ggplot(data, aes(x = data[,2], y=data[,3])) +geom_point(size = input$var2,colour =input$var5, shape=input$var6) + 
            ...
            if(input$plot_type == "1" ){
              p1 <- ggMarginal(p, type = "histogram", fill = input$var1, binwidth = input$var7)
            }
            if(input$plot_type == "2"){
              p1 <- ggMarginal(p, type = "density", color = input$var1, size=4)
            }
            if(input$plot_type == "3" ){
              if(input$outlier == 1){
                p1 <- ggMarginal(p, type = "boxplot", color = input$var1)
              }
              if(input$outlier == 2){
                p1 <- ggMarginal(p, type = "boxplot", color = input$var1, outlier.colour = NA)
              }
            }
            return(p1)
          }
